...
OUT_ROOT="$REPO_ROOT/tests/tmp/sec002-output"
...
label="$(jq -r '.security_findings[0].label' "$json_path")"
snippet="$(jq -r '.security_findings[0].snippet' "$json_path")"
if [[ "$label" != "Dangerous code execution function" ]]; then
	echo "Unexpected finding label: $label"
	exit 1
fi
if [[ "$snippet" != 'exec("ls -la");' ]]; then
	echo "Unexpected finding snippet: $snippet"
	exit 1
fi

echo "SEC-002 exec precision regression test passed"
